...
- # Notes
	- ## First do this
	  collapsed:: true
		...
		- read some documentation and papers about [[PVS]] and [[dL]] (See #resources)
	...
	- ## Writing things in [[PVS]] and [[dL]]
	  collapsed:: true
		- ### Creating and Referencing Variables
		  collapsed:: true
			- Constants can be defined as [[PVS]] variables like so, and must be referenced with a `cnst( )` around them #cnst #constants
			  collapsed:: true
				- `x: VAR real`
			- Variables are defined as natural numbers with UNIQUE arbitrary values that correspond to their index, and must be referenced with a `val( )` around them #val #variables
			  collapsed:: true
				- `x: nat = 0`
		- `IMPORTING dL@top`: #IMPORTING
		- `DIFF()`: #DIFF
		- `LEMMA` #lemma
		- `UNION()` #UNION
		- `IFTE()` #IFTE
		- `(BETA)` #beta
		- `TEST()`: #[[test]]
		- `div_safe_re()` #div_safe_re
		- `SEQ(HP, HP)` #SEQ
		- `STAR(arg)` #star
		- `^^`: #exponentiation
		- `ALLRUNS(DIFF(), postcondition)` #ALLRUNS
		- `SOMERUNS(DIFF(), postcondition)` #SOMERUNS
